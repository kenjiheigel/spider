...
function Spider(domain, file) {
	...
	if (casper.cli.has('depth')) {
		instance.depth = casper.cli.get('depth');
	}
	else {
		instance.depth = conf.depth;
	}
	...
	instance.ic = {};
	instance.checkDuplicate = {};

	for(var i = 0; i < conf.viewport.length; i++) {
		instance.ic[conf.viewport[i].name] = new ImageComparator();
		instance.checkDuplicate[conf.viewport[i].name] = instance.ic[conf.viewport[i].name].init();
	}

	fs.write(instance.fname, '', 'w');

	casper.on('screenshot.saved', function(viewport, file) {
		instance.checkDuplicate[viewport] = instance.checkDuplicate[viewport].then(
			function() {
				return instance.ic[viewport].isDuplicate(file, fs.absolute('screenshots/' + viewport + '/junk'));
			},
			function(err) {
				console.log(err);
